    ...
    public class IrcConnection
    {
        ...
        public void WriteLine(string data, Priority priority)
        {
            if (priority == Priority.Critical) {
                ...
                _WriteThread.SendBuffer[priority].Enqueue(data);                
        ...
        private class WriteThread
        {
            ...
            public ConcurrentDictionary<Priority, ConcurrentQueue<String>> SendBuffer;
...
#region WARNING: complex scheduler, don't even think about changing it!
            ...
            {
                _HighCount        = SendBuffer[Priority.High].Count;
                _AboveMediumCount = SendBuffer[Priority.AboveMedium].Count;
                _MediumCount      = SendBuffer[Priority.Medium].Count;
                _BelowMediumCount = SendBuffer[Priority.BelowMedium].Count;
                _LowCount         = SendBuffer[Priority.Low].Count;
            ...
            private bool _CheckHighBuffer()
            {
                if (_HighCount > 0) {
                    ...
                    if (SendBuffer[Priority.High].TryDequeue(out data) || _Connection._WriteLine(data) == false) {
#if LOG4NET
                        Logger.Queue.Warn("Sending data was not sucessful, data is requeued!");
#endif
                        SendBuffer[Priority.High].Enqueue(data);
                        return false;
                    }
            ...
            private bool _CheckAboveMediumBuffer()
            {
                if ((_AboveMediumCount > 0) &&
                    ...
                    string data = string.Empty;
                    if (!SendBuffer[Priority.AboveMedium].TryDequeue(out data) || _Connection._WriteLine(data) == false) {
#if LOG4NET
                        Logger.Queue.Warn("Sending data was not sucessful, data is requeued!");
#endif
                        SendBuffer[Priority.AboveMedium].Enqueue(data);
                        return false;
                    }
            ...
            private bool _CheckMediumBuffer()
            {
                if ((_MediumCount > 0) &&
                    ...
                    string data = string.Empty;
                    if (!SendBuffer[Priority.Medium].TryDequeue(out data) || _Connection._WriteLine(data) == false) {
#if LOG4NET
                        Logger.Queue.Warn("Sending data was not sucessful, data is requeued!");
#endif
                        SendBuffer[Priority.Medium].Enqueue(data);
                        return false;
                    }
                    _MediumSentCount++;
            ...
            private bool _CheckBelowMediumBuffer()
            {
                if ((_BelowMediumCount > 0) &&
                    ...
                    string data = string.Empty;
                    if (!SendBuffer[Priority.BelowMedium].TryDequeue(out data) || _Connection._WriteLine(data) == false) {
#if LOG4NET
                        Logger.Queue.Warn("Sending data was not sucessful, data is requeued!");
#endif
                        SendBuffer[Priority.BelowMedium].Enqueue(data);
                        return false;
                    }
            ...
            private bool _CheckLowBuffer()
            {
                if (_LowCount > 0) {
                    ...
                    string data = string.Empty;
                    if (!SendBuffer[Priority.Low].TryDequeue(out data) || _Connection._WriteLine(data) == false) {
#if LOG4NET
                        Logger.Queue.Warn("Sending data was not sucessful, data is requeued!");
#endif
                        SendBuffer[Priority.Low].Enqueue(data);
                        return false;
                    }
